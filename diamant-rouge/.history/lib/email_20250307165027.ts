...
export async function  sendOrderUpdateEmail(to: string, orderId: number, status: string) {
  const subject = `Mise à jour de votre commande #${orderId} - Diamant Rouge`;
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #333;">
      ...
      <h1 style="color: #9a2240; text-align: center; margin-bottom: 20px;">Mise à jour de votre commande</h1>
      
      <p>Bonjour,</p>
      
      <p>Votre commande #${orderId} a été mise à jour avec le statut : <strong>${status}</strong>.</p>
      
      <p>Vous pouvez consulter les détails de votre commande en vous connectant à votre compte sur notre site.</p>
