...
export function ManageProducts() {
  ...
  {(isEdit ? editingProduct.images : newProduct.images)?.map((img, idx) => (
    <motion.div
      ...
      className="relative group"
    >
      <div className="aspect-square w-full relative overflow-hidden rounded-lg shadow-luxury border border-brandGold/10 group-hover:border-brandGold/40 group-hover:shadow-deepGlow transition-all duration-300">
        <Image
          src={img}
          alt={`Image ${idx + 1}`}
          fill
          ...
          sizes="(max-width: 768px) 33vw, 25vw"
        />
        <div className="absolute inset-0 bg-gradient-to-br from-transparent to-richEbony/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
      </div>
      <button
        type="button"
        onClick={() => removeImage(idx, isEdit)}
        className="absolute -top-2 -right-2 bg-burgundy text-white rounded-full w-6 h-6 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 shadow-subtle hover:shadow-md"
      >
        ×
      </button>
      <div className="absolute bottom-2 left-2 bg-brandGold/80 text-richEbony text-xs px-2 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        Image {idx + 1}
      </div>
    </motion.div>
  ))}
  
  {/* Empty placeholder with luxury feel */}
  {(!isEdit ? !newProduct.images || newProduct.images.length === 0 : !editingProduct.images || editingProduct.images.length === 0) && (
    <div className="aspect-square w-full flex flex-col items-center justify-center border border-dashed border-brandGold/30 rounded-lg bg-gradient-to-br from-brandIvory to-white text-platinumGray">
      <Diamond className="text-brandGold/40 mb-2" size={28} />
      <span className="text-sm">Aucune image</span>
      <span className="text-xs mt-1 px-4 text-center">Ajoutez des images pour mettre en valeur votre création</span>
    </div>
  )}
</div>
                                                                        </div>
