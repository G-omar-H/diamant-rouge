...
async function handleHeroImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
  ...
  try {
    ...
    const data = await res.json();
    // The API returns { urls: ["/uploads/filename.jpg"] }
...
async function handleContentImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
  ...
  try {
    ...
    // Fix endpoint to match the hero image endpoint
    const res = await fetch("/api/admin/upload-image", {
      method: "POST",
      body: formData,
    });
    ...
    // Insert image in the editor with absolute URL
    const siteUrl = window.location.origin; // Get the site's base URL
    const absoluteUrl = imageUrl.startsWith('http') ? imageUrl : `${siteUrl}${imageUrl}`;
    const imageHtml = `<img src="${absoluteUrl}" alt="${newImage.alt}" class="newsletter-content-image" style="max-width: 100%; margin: 1rem 0;" />`;
  ...
  function generateNewsletterHtml() {
    ...
    const heroImageUrl = heroImage?.url ? 
      (heroImage.url.startsWith('http') ? heroImage.url : `${siteUrl}${heroImage.url}`) : '';
    ...
    <body>
      <div class="container">
        <div class="header">
          <img src="${siteUrl}/logo.png" alt="Diamant Rouge" class="logo">
        </div>
        
        ${heroImage ? `<img src="${heroImageUrl}" alt="${heroImage.alt}" class="hero-image">` : ''}
        ...
        <div class="footer">
          ...
          <p><a href="${siteUrl}/unsubscribe" style="color: #d4af37;">Se désabonner</a></p>
        </div>
      </div>
    </body>
    </html>
        ...
        <div className="bg-white/50 backdrop-blur-sm border border-brandGold/20 rounded-lg overflow-hidden shadow-luxury">
          ...
          <form onSubmit={handleSendMassEmail} className="space-y-6">
            ...
            <div>
              ...
              {heroImage && (
  <div className="relative rounded-md overflow-hidden border border-brandGold/20">
                  ...
                  <div className="border border-brandGold/20 rounded-md overflow-hidden">
                    ...
                    <div className="p-3 max-h-[600px] overflow-auto bg-white">
                      <div className="shadow-md">
                        ...
                        <div className="p-6">
                          <h2 className="text-xl font-serif mb-4">{subject || "Sujet de la newsletter"}</h2>
                          <div dangerouslySetInnerHTML={{ __html: htmlContent || "<p>Contenu de la newsletter...</p>" }} />
                        </div>
                        
                        <div className="bg-richEbony p-4 text-center text-brandGold text-xs">
                          <p>© {new Date().getFullYear()} Diamant Rouge. Tous droits réservés.</p>
                        </div>
                      </div>
                    </div>
