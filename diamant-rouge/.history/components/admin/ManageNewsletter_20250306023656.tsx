...
import { FiCheckSquare, FiSquare, FiAlertCircle, FiCheck, FiSend, FiMail, FiUsers, FiCalendar, FiImage, FiPlusCircle, FiEye, FiTrash2, FiLayout } from "react-icons/fi";
...
export function ManageNewsletter() {
  ...
  async function handleContentImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // Insert image URL into the editor at cursor position
      const imageHtml = `<img src="${newImage.url}" alt="${newImage.alt}" class="newsletter-content-image" style="max-width: 100%; margin: 1rem 0;" />`;
      setHtmlContent(prev => {
        // Simple insertion at the end if we can't determine cursor position
        return prev + imageHtml;
      });
  ...
  async function handleSendMassEmail(e: React.FormEvent) {
    ...
    // Generate full HTML for the newsletter
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white/50 backdrop-blur-sm border border-brandGold/20 rounded-lg p-6 shadow-luxury lg:col-span-2">
          ...
          <form onSubmit={handleSendMassEmail} className="space-y-6">
            ...
            {/* Rich Text Editor */}
            <div>
              <label className="block text-sm font-medium text-richEbony mb-2 flex justify-between items-center">
            ...
            <div className="flex justify-end pt-4">
              <button
                ...
                disabled={isSending || loading || uploadingImage}
              >
                {isSending ? (
                  <>
                    <div className="w-4 h-4 border-2 border-t-transparent border-richEbony/50 rounded-full animate-spin"></div>
