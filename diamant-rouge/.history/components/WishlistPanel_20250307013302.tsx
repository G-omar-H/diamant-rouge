    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          {/* Wishlist panel */}
          <motion.div 
  className="fixed top-0 right-0 h-full w-full sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl 2xl:max-w-2xl bg-white z-50 shadow-luxury flex flex-col"
  initial={{ x: '100%' }}
  animate={{ x: 0 }}
  exit={{ x: '100%' }}
...
>
            {/* Header */}
            <div className="border-b border-platinumGray/10 p-5 flex items-center justify-between">
              <div className="flex items-center">
                <h3 className="font-serif text-lg text-richEbony">Mes Favoris</h3>
                {wishlist.length > 0 && (
              ...
              <button 
                onClick={onClose}
                className="p-1 rounded-full hover:bg-richEbony/5 transition-colors"
                aria-label="Fermer"
              >
