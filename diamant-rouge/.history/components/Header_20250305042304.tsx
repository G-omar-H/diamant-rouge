...
export default function Header() {
  ...
  return (
    <header className="w-full bg-brandIvory z-40">
      <div className="max-w-screen-xl mx-auto px-6 py-6 md:py-8 flex flex-col">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-10">
            ...
            >
              ...
              <span className="text-xs font-medium tracking-wide">Rendez-vous</span>
            </Link>
          </div>

          {/* Center: Enlarged Logo with increased margins */}
          <div className="flex-grow flex justify-center items-center mx-10 md:mx-16">
            <Link
              href="/"
              title="Accueil Diamant Rouge"
              className="inline-block hover:scale-105 transition-transform duration-300 py-2 md:py-4"
            >
              <Image
          ...
          <div className="flex items-center gap-10">
            <Link href="/wishlist" title="Votre liste de souhaits">
              <div className="relative flex flex-col items-center hover:text-burgundy transition-colors duration-300">
                ...
                <span className="text-xs font-medium tracking-wide">Liste</span>
                {wishlist.length > 0 && (
            ...
            <Link href="/cart" title="Votre panier">
              <div className="relative flex flex-col items-center hover:text-burgundy transition-colors duration-300">
                ...
                <span className="text-xs font-medium tracking-wide">Panier</span>
                {cart.length > 0 && (
                  <span className="absolute -top-2 -right-2 bg-burgundy text-brandIvory text-xs px-1.5 py-0.5 rounded-full">
                    {cart.length}
                  </span>
            ...
            {session ? (
              <>
                <Link href="/profile" title="Votre Profil">
                  <div className="flex flex-col items-center hover:text-burgundy transition-colors duration-300">
                    ...
                    <span className="text-xs font-medium tracking-wide">Profil</span>
                  </div>
                </Link>
                <button
                  onClick={() => signOut()}
                ...
                >
                  ...
                  <span className="text-xs font-medium tracking-wide">Déconnexion</span>
                </button>
              </>
            ) : (
              <Link href="/auth" title="Se connecter">
                <div className="flex flex-col items-center hover:text-burgundy transition-colors duration-300">
                  <User size={22} className="text-brandGold mb-1.5" />
                  <span className="text-xs font-medium tracking-wide">Connexion</span>
                </div>
              </Link>
            )}
            {/* Mobile Nav Toggle */}
        ...
        <nav className="hidden md:flex justify-center mt-8 text-sm tracking-wider font-medium text-richEbony">
          <Link
            href="/"
            className="px-10 text-center hover:text-brandGold transition-colors duration-300"
          >
            Accueil
          </Link>
          <Link
